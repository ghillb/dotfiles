...
nvim() {
  ...
  wget -nc https://github.com/neovim/neovim/releases/download/v0.6.0/${release}.tar.gz -O - > ${release}
  sudo tar -xf ${release} -C /tmp
  sudo cp -r /tmp/${release}/* /usr/local
  sudo rm -rf ${release} /tmp/${release}
  $(which nvim) --headless -u .files/neovim/.config/nvim/utils/plugins.lua -c 'autocmd User PackerComplete quitall' -c 'PackerSync'
